...
import com.crdcs.workbench.model.Login;
import com.crdcs.workbench.model.Result;
...
public class UserController {
    ...
    public Result queryUserByPagination(UserQueryCondition condition, HttpServletRequest request){
        ...
        return Result.build(1,"用户分页信息",userService.userPage(condition));
    }

    @GetMapping("/user/{id}")
    public Result getUser(@PathVariable("id") long id) {
        return Result.build(1,"用户信息",userService.user(id));
    }
    ...
    public Result changeUserRemoved(@RequestBody UserParam userParam){
        userService.changeUserRemoved(userParam.getId(), userParam.getRemoved());
    ...
    public void exportUser(@RequestBody UserQueryCondition condition,HttpServletResponse response){
        ...
        try {
            List<UserParam> list = userService.exportUser(condition);
    ...
    public Result importUser(MultipartFile file) {
        try {
            ...
            userService.batchAddUser(correctList);
